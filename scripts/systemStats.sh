...
# ? ** This script will only work on real DashLab hardware since it reads data from the RPi sensors and VideoCore GPU **
# It's a complete mess but it works. Returns JSON on stdout for easy parsing by the DashLab Node app
# Requires the "bc" command line calculator package.
